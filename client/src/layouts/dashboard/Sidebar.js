...
const AntSwitch = styled(Switch)(({ theme }) => ({
    ...
    '& .MuiSwitch-track': {
        ...
        opacity: 1,
        backgroundColor:
            theme.palette.mode === 'dark' ? 'rgba(255,255,255,.35)' : 'rgba(0,0,0,0.25)',
...
const Sidebar = () => {
    ...
    return (
        <>
            <Box p={2} sx={{ backgroundColor: theme.palette.background.default, height: "100vh", boxShadow: "0px 0px 2px rgba(0, 0, 0, 0.25)", width: 100 }}>
                <Stack spacing={2} direction="row" sx={{ width: "100%", height: "100%" }} alignItems={"center"} justifyContent="space-between">
                    <Stack spacing={3} direction="column" sx={{ width: "100%", height: "100%" }} alignItems={"center"} justifyContent="space-between">
                        <Stack alignItems={"center"} spacing={4}>
                            <Box sx={{ backgroundColor: theme.palette.primary.main, height: 64, width: 64, borderRadius: 12 }}>
                                <img src={Logo} alt="logo.ico" />
                            </Box>
                            <Stack sx={{ width: "max-content" }} direction="column" alignItems="center" spacing={3}>
                                {Nav_Buttons.map((el) => (
                                    el.index === selected ?
                                        <Box sx={{ backgroundColor: theme.palette.primary.main, borderRadius: 12, }}>
